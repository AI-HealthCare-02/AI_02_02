...
from backend.services.rag import RAGResult, RAGService
from backend.services.rag_corpus import (
    CorpusDocument,
    CorpusIndex,
    CorpusSnippet,
    _tokenize,
...
class TestSanitize:
    ...
    def test_remove_internal_path(self):
        svc = self._svc()
        text = "이 내용은 backend/services/chat.py에서 사용됩니다"
        result = svc._sanitize(text)
        assert "backend/services/chat.py" not in result
