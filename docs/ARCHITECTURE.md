...
|
+-- apps/web/             <-- 프론트엔드 (Next.js 14)
+-- ai_worker/            <-- AI 모델 전용 공간
+-- docs/                 <-- 문서 (지금 읽고 있는 곳!)
|   +-- planning/         <-- 기획서 (읽기 전용!)
...
```
1단계(원천): 매일수집질문_DB설계.md, 온보딩설문플랜.md
     |
2단계(파생): DANAA_DB명세최종확정안.md
     |
3단계(계약): DANAA_API최종확정안.md
     |
4단계(참조): HTML 프로토타입
     |
5단계(코드): app/models/, app/services/
```

DB 테이블 구조가 궁금하면 > `docs/collaboration/DANAA_DB명세최종확정안_2026-04-02.md` 참고
